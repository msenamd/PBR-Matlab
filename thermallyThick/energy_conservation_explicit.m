...
function [temp] = energy_conservation_explicit(dt_old,q_surf_old, ...
...
i = 1;
FO_L = 0;
FO_R = 0.5*(kp(i)+kp(i+1))/rho_times_cp(i) * S_pos(i)*dt_old / ...
                    (dV(i)*(xCenter(i+1)-xCenter(i)));

temp(i) = (1-FO_R)*temp_old(i) + FO_R*temp_old(i+1) ...
                                + dt_old/rho_times_cp(i)*Qdotp(i);

% i = nx_old (exposed surface)
i = nx_old;
FO_L = 0.5*(kp(i)+kp(i-1))/rho_times_cp(i) * S_neg(i)*dt_old / ...
                                     (dV(i)*(xCenter(i)-xCenter(i-1)));
FO_R = 0;

temp(nx_old) = FO_L*temp_old(nx_old-1) + (1-FO_L)*temp_old(nx_old) ...
             + dt_old/rho_times_cp(i)*S_pos(nx_old)/dV(nx_old)*q_surf_old ...
             + dt_old/rho_times_cp(i)*Qdotp(nx_old) ;
...
for i=2:(nx_old-1)
    ...
    temp(i) = FO_L*temp_old(i-1)+(1-FO_L-FO_R)*temp_old(i) ...
