...
for i = 1:nx_old
    ...
    %    keff = k_s + psi x k_g
    k_ws    = k0_ws*(temp_old(i)/300)^nk_ws;
    k_ds    = k0_ds*(temp_old(i)/300)^nk_ds;
    k_c     = k0_c *(temp_old(i)/300)^nk_c;
    k_a     = k0_a *(temp_old(i)/300)^nk_a;
    k_s     = k_ws*x_ws_old(i) + k_ds*x_ds_old(i) ...
            + k_c * x_c_old(i) + k_a * x_a_old(i); % Conduct. in solid p.
    gamma   = gamma_ws*x_ws_old(i) + gamma_ds*x_ds_old(i) ...
            + gamma_c *x_c_old(i)  + gamma_a *x_a_old(i);
    k_rad   = gamma*sigma*temp_old(i)^3;
    k_s     = k_s + k_rad;
    k_g     = k_g0*(temp_old(i)/300)^(0.76);    % Conductivity in gas phase
    keff(i) = k_s + psi*k_g; % Estimated at time t(n)
    
    % Product of mass density times specific heat
    %  - Porous medium treatment:
    %    rhocp_eff = rhocp_s + psi x rho_g x cp_g
    c_ws         = c0_ws*(temp_old(i)/300)^nc_ws;
    c_ds         = c0_ds*(temp_old(i)/300)^nc_ds;
    c_c          = c0_c *(temp_old(i)/300)^nc_c;
    c_a          = c0_a *(temp_old(i)/300)^nc_a;
    rhocp_s      = rho_ws*(1-psi_ws)*c_ws*x_ws_old(i) ...
                 + rho_ds*(1-psi_ds)*c_ds*x_ds_old(i) ...
                 + rho_c *(1-psi_c) *c_c * x_c_old(i) ...
                 + rho_a *(1-psi_a) *c_a * x_a_old(i); % (rho*cp) in sol p. 
    rhocp_g      = (pres_g*MW_g/R/temp_old(i))*cp_g0; % (rho*cp) in gas p.
    rhocp_eff(i) = rhocp_s + psi*rhocp_g; % Estimated at time t(n)
    ...
    dRRTemp(i) = Qdotp*dt/rhocp_eff(i)/dV_old(i);
    bRRTemp(i) =-Qdotm*dt/rhocp_eff(i)/temp_olditer(i)/dV_old(i);
end
