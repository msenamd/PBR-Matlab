...
for i = 1:nx_old
    ...
    xws_dV_old     = ( x_ws_old(i)    *dV_old(i) );
    xws_dV_olditer = ( x_ws_olditer(i)*dV_old(i) );
    RHSp                 =  0;
    RHSm                 = -K_R1/rho_ws/(1-psi_ws);
    
    xws_dV_newiter = ...
        ( xws_dV_old + dt*RHSp + lambda*xws_dV_olditer ) ...
        /( 1 - dt*RHSm/max(xws_dV_olditer,1e-14) + lambda );
    
    xds_dV_old     = ( x_ds_old(i)    *dV_old(i) );
    xds_dV_olditer = ( x_ds_olditer(i)*dV_old(i) );
    RHSp                 =  K_R1*eta_ds_R1/rho_ds/(1-psi_ds);
    RHSm                 = -(K_R2+K_R3)/rho_ds/(1-psi_ds);
    
    xds_dV_newiter = ...
        ( xds_dV_old + dt*RHSp + lambda*xds_dV_olditer ) ...
        /( 1 - dt*RHSm/max(xds_dV_olditer,1e-14) + lambda );
    
    xc_dV_old      = ( x_c_old(i)     *dV_old(i) );
    xc_dV_olditer  = ( x_c_olditer(i) *dV_old(i) );
    RHSp                 =  K_R2*eta_c_R2/rho_c/(1-psi_c) ...
                          + K_R3*eta_c_R3/rho_c/(1-psi_c);
    RHSm                 = -K_R4/rho_c/(1-psi_c);
    
    xc_dV_newiter = ...
        ( xc_dV_old + dt*RHSp + lambda*xc_dV_olditer ) ...
        /( 1 - dt*RHSm/max(xc_dV_olditer,1e-14) + lambda );
    
    
    xa_dV_old      = ( x_a_old(i)     *dV_old(i) );
    xa_dV_olditer  = ( x_a_olditer(i) *dV_old(i) );
    RHSp                 =  K_R4*eta_a_R4/rho_a/(1-psi_a);
    RHSm                 =  0;
    
    xa_dV_newiter = ...
        ( xa_dV_old + dt*RHSp + lambda*xa_dV_olditer ) ...
        /( 1 - dt*RHSm/max(xa_dV_olditer,1e-14) + lambda );
    
    
    % Update cell volume
    dV(i) = xws_dV_newiter + xds_dV_newiter ...
          +  xc_dV_newiter +  xa_dV_newiter;
    
    % Update mole fractions
    x_ws(i) = xws_dV_newiter/dV(i);
    x_ds(i) = xds_dV_newiter/dV(i);
    x_c(i)  =  xc_dV_newiter/dV(i);
    x_a(i)  =  xa_dV_newiter/dV(i);
