...
function [q_surf, temp_surf] = particle_surface(temp_surf_old, ...
                            keff_surf, eps_surf, dx_surf, tempc_nx, h_conv)
                      
global T_g G sigma

% Calculation of net surface heat flux and surface temperature 
...
while (small > 1e-3 && iter <= maxIter)
    ...
    T_new = ( tempc_nx + (h_conv*T_g+eps_surf*G)*dx_surf/keff_surf ) ...
            /(1+Bi);
...
if(iter >= maxIter)
    fprintf(' *** Error in particle_surface \n');
    fprintf(' Iterations exceed the maximum limit, temp_surf = %g \n', ...
            T_new);
    return
end
