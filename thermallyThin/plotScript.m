...
%% >>>>>>>>>>>>> START Genral Plots <<<<<<<<<<<<< 
figure(1);   % Temperature vs times
m=1;
plot(time_model(1:n_output),T_model(1:n_output),'-b','linewidth',1.2);
title('Particle Temperature Variation with Time','fontsize',12,'Interpreter','latex');
...
AA=plot(time_model(1:n_output),x_m_model(1:n_output),'-r','linewidth',1.2);
hold on;
BB=plot(time_model(1:n_output),x_vs_model(1:n_output),'-k','linewidth',1.2);
CC=plot(time_model(1:n_output),x_c_model(1:n_output),'-b','linewidth',1.2);
